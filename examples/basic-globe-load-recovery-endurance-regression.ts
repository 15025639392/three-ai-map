...
function createImageryTile(coordinate: TileCoordinate): HTMLCanvasElement {
  ...
  if (!context) {
    throw new Error("Missing 2D canvas context for basic load-recovery-endurance tile");
  }
...
export function runBasicGlobeLoadRecoveryEnduranceRegression(
  ...
  const rasterSource = new RasterTileSource("imagery", {
    tiles: ["imagery://{z}/{x}/{y}"],
    tileSize: 256,
    minZoom: 0,
    maxZoom: 22,
  ...
  engine.addSource("imagery", rasterSource);
  const imageryLayer = new RasterLayer({ id: "imagery", source: "imagery", opacity: 1 });
  ...
  const finalize = async (): Promise<void> => {
    try {
      ...
      const heavyInteractionStepsPerCycle = 5;
      const recoveryInteractionStepsPerCycle = 4;
      let layerRecoveredCount = 0;
      let sceneObjectRecoveredCount = 0;
      let renderRecoveredCount = 0;
      let heavyImageryRequestedTotal = 0;
      let recoveryImageryRequestedTotal = 0;
      let heavyInteractionStepCountTotal = 0;
      let recoveryInteractionStepCountTotal = 0;
      let recoveryTileMatchCount = 0;
      let heavyAfterTiles = "";
      let recoveryAfterTiles = "";
      let firstRecoveryTileKey: string | null = null;
      ...
      for (let cycleIndex = 0; cycleIndex < cycleCount; cycleIndex += 1) {
        ...
        const heavyViewSequence = [
          ...
          { lng: 124 - cycleIndex * 9, lat: 22, altitude: 1.1, waitMs: 90 },
          { lng: 88 - cycleIndex * 5, lat: 16, altitude: 1.24, waitMs: 100 }
        ];
        await runViewSequence(engine, heavyViewSequence);
        heavyInteractionStepCountTotal += heavyViewSequence.length;
        await terrain.ready();
        await sleep(210);
        const heavyMetrics = collectProfileMetrics(engine, terrain, rasterSource, heavyStatsBefore);
        heavyAfterTiles = terrain.getActiveTileKeys().join(",");
        ...
        engine.resetPerformanceReport();
        const recoveryViewSequence = [
          { lng: 42, lat: 28, altitude: 2.12, waitMs: 90 },
          { lng: -6, lat: 29, altitude: 2.28, waitMs: 90 },
          { lng: 68, lat: 22, altitude: 2.42, waitMs: 90 },
          { lng: 86, lat: 18, altitude: 2.48, waitMs: 110 }
        ];
        await runViewSequence(engine, recoveryViewSequence);
        recoveryInteractionStepCountTotal += recoveryViewSequence.length;
        await terrain.ready();
        await sleep(180);
        const recoveryMetrics = collectProfileMetrics(engine, terrain, rasterSource, recoveryStatsBefore);
        recoveryAfterTiles = terrain.getActiveTileKeys().join(",");
        if (firstRecoveryTileKey === null) {
          firstRecoveryTileKey = recoveryAfterTiles;
        }
        if (recoveryAfterTiles === firstRecoveryTileKey) {
          recoveryTileMatchCount += 1;
        }
        ...
        const renderRecovered = Number(recoveryMetrics.renderCount <= heavyMetrics.renderCount);
        layerRecoveredCount += layerRecovered;
        sceneObjectRecoveredCount += sceneObjectRecovered;
        renderRecoveredCount += renderRecovered;
      }
      ...
      const recoveryTileStableCount = Number(recoveryTileMatchCount === cycleCount);
      ...
      const allExpected = Number(
        layerRecoveredCount === cycleCount &&
          ...
          renderRecoveredCount >= cycleCount - 1 &&
          stableRecoverySceneObjectCount === 1 &&
          recoveryTileStableCount === 1 &&
          heavyInteractionStepCountTotal === cycleCount * heavyInteractionStepsPerCycle &&
          recoveryInteractionStepCountTotal === cycleCount * recoveryInteractionStepsPerCycle &&
      ...
      frameLoopStopped = true;
      container.dataset.phase = "after-load-recovery-endurance";
      ...
      setDataAttribute(container, "renderRecoveredCount", renderRecoveredCount);
      setDataAttribute(container, "stableRecoverySceneObjectCount", stableRecoverySceneObjectCount);
      setDataAttribute(container, "recoveryTileStableCount", recoveryTileStableCount);
      setDataAttribute(container, "heavyInteractionStepCountTotal", heavyInteractionStepCountTotal);
      setDataAttribute(container, "recoveryInteractionStepCountTotal", recoveryInteractionStepCountTotal);
      ...
      setDataAttribute(container, "metricsScenario", "basic-load-recovery-endurance");
      ...
      output.textContent = `after-load-recovery-endurance:${recoveryAfterTiles || "none"}:layerRecovered=${layerRecoveredCount}/${cycleCount}:sceneRecovered=${sceneObjectRecoveredCount}/${cycleCount}`;
    } catch (error) {
      handleError(error);
    }
  };
  ...
  container.dataset.renderRecoveredCount = "";
  container.dataset.stableRecoverySceneObjectCount = "";
  container.dataset.recoveryTileStableCount = "";
  container.dataset.heavyInteractionStepCountTotal = "";
  container.dataset.recoveryInteractionStepCountTotal = "";
  ...
  output.textContent = "启动中:basic-globe-load-recovery-endurance-regression";
  ...
  if (typeof window !== "undefined") {
    (
      window as Window & {
        __basicGlobeLoadRecoveryEnduranceRegression?: {
          engine: GlobeEngine;
          terrain: TerrainTileLayer;
          rasterSource: RasterTileSource;
          imageryLayer: RasterLayer;
        };
      }
    ).__basicGlobeLoadRecoveryEnduranceRegression = {
      engine,
...
export function bootstrap(): void {
  ...
  app.innerHTML = `
    <main class="demo-shell">
      ...
      <div class="demo-status" id="status-output">启动中:basic-globe-load-recovery-endurance-regression</div>
    </main>
  `;
  ...
  if (stage && status) {
    runBasicGlobeLoadRecoveryEnduranceRegression(stage, status);
  }
}
