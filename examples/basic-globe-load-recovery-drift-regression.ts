...
function createImageryTile(coordinate: TileCoordinate): HTMLCanvasElement {
  ...
  if (!context) {
    throw new Error("Missing 2D canvas context for basic load-recovery-drift tile");
  }
...
export function runBasicGlobeLoadRecoveryDriftRegression(
  ...
  const finalize = async (): Promise<void> => {
    try {
      ...
      frameLoopStopped = true;
      container.dataset.phase = "after-load-recovery-drift";
      ...
      setDataAttribute(container, "metricsScenario", "basic-load-recovery-drift");
      ...
      output.textContent = `after-load-recovery-drift:${recoveryAfterTiles || "none"}:layerRecovered=${layerRecoveredCount}/${cycleCount}:sceneRecovered=${sceneObjectRecoveredCount}/${cycleCount}`;
    } catch (error) {
      handleError(error);
    }
  };
  ...
  output.textContent = "启动中:basic-globe-load-recovery-drift-regression";
  ...
  if (typeof window !== "undefined") {
    (
      window as Window & {
        __basicGlobeLoadRecoveryDriftRegression?: {
          engine: GlobeEngine;
...
export function bootstrap(): void {
  ...
  app.innerHTML = `
    <main class="demo-shell">
      ...
      <div class="demo-status" id="status-output">启动中:basic-globe-load-recovery-drift-regression</div>
    </main>
  `;
  ...
  if (stage && status) {
    runBasicGlobeLoadRecoveryDriftRegression(stage, status);
  }
}
